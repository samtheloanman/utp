/* Professional line-icon system for Uniting the People. No emoji.
   <Icon name="..." size={18} /> — 24x24 stroke icons inherit currentColor. */
...
/* The brand mark — ring cradling two spheres, flat brand build. */
function BrandMark({ size = 34, reversed = false }) {
...
/* Flag: image for countries, globe icon for supranational/global. */
function Flag({ cc, size = 18, round = true }) {
  if (!cc) {
    return <span className="flag-globe" style={{ width: size, height: size }}><Icon name="globe" size={Math.round(size * 0.78)} stroke={1.7} /></span>;
  }
  return <img className="flag-img" src={window.flagUrl(cc, 80)} alt=""
    style={{ width: size, height: round ? size : Math.round(size * 0.72), borderRadius: round ? '50%' : 3 }}
    loading="lazy" referrerPolicy="no-referrer" />;
}

Object.assign(window, { Icon, BrandMark, Flag, CAT_COLOR, ICON_PATHS });
