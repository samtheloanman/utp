/* Shared UI components for Uniting the People */
...
function scaleMeta(key) { return window.SCALES.find(s => s.key === key) || {}; }

/* Photo with graceful gradient + icon fallback */
function Media({ id, cat, ratio = '16 / 9', children }) {
  const [ok, setOk] = useState(true);
  const color = (window.CAT_COLOR && window.CAT_COLOR[cat]) || '#0E7C96';
  return (
    <div className="media" style={{ '--cat': color, aspectRatio: ratio }}>
      <div className="media-bg" />
      <div className="media-watermark"><Icon name={cat} size={64} stroke={1.3} /></div>
      {id && ok && (
        <img className="media-img" src={window.photoUrl(id, 700)} alt="" loading="lazy"
...
function Sparkline({ data, w = 64, h = 24, color }) {
...
function SentimentBar({ issue, showLegend = true }) {
  return (
    <div>
      <div className="sbar">
    ...
    <div className="chart-wrap">
      <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h} preserveAspectRatio="none" style={{ overflow: 'visible' }}>
        <defs><linearGradient id="trendFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="var(--for)" stopOpacity="0.20" /><stop offset="100%" stopColor="var(--for)" stopOpacity="0" />
        </linearGradient></defs>
...
function QuickVote({ issue, vote, onVote }) {
  return (
...
function IssueCard({ issue, vote, onVote, onOpen }) {
    ...
    <article className="icard" onClick={() => onOpen(issue.id)}>
      <Media id={issue.img} cat={issue.category}>
        ...
        <div className="media-region">
          <Flag cc={issue.cc} size={18} />{issue.region}
        </div>
      </Media>
...
function TopBar({ query, onQuery, signedIn, onSignIn, onHome }) {
  return (
    <header className="topbar">
      <div className="topbar-inner">
        ...
        <div className="searchbox">
          ...
          <input value={query} onChange={(e) => onQuery(e.target.value)} placeholder="Search issues, places, topics" />
        </div>

        <div className="topbar-actions">
          <button className="btn btn-quiet btn-sm"><Icon name="shield" size={15} stroke={1.9} />How it works</button>
          {signedIn
            ? <button className="btn btn-ghost btn-sm"><span className="avatar">YO</span>You</button>
            : <button className="btn btn-primary btn-sm" onClick={onSignIn}>Sign in to vote</button>}
        </div>
      </div>
    </header>
  );
...
Object.assign(window, { fmt, scaleMeta, Media, Sparkline, SentimentBar, TrendChart, QuickVote, IssueCard, TopBar });
