...
export const CATEGORIES = ['All', 'Climate', 'Health', 'Economy', 'Rights', 'Technology', 'Governance', 'Education'];

function series(start: number, end: number, n = 14, jitter = 3) {
  const out = [];
  for (let i = 0; i < n; i++) {
    const t = i / (n - 1);
...
export function makeActivity(issue: Issue) {
  const verbs: Record<string, string> = { for: 'voted For', against: 'voted Against', unsure: 'is Unsure' };
...
export function photoUrl(id: string, w?: number) { 
  return IMG_BASE + id + '?auto=format&fit=crop&w=' + (w || 800) + '&q=70'; 
}
